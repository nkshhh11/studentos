...
export default function ProjectsPage() {
  const { projects, addProjectTask, toggleProjectTask } = useStudentOS();
    ...
    <div className="space-y-6 pb-12 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-zinc-800">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2">
            <FolderGit2 className="w-6 h-6 text-teal-400" /> Student Projects & GitHub Manager
          </h1>
      ...
      {analysisResult && (
        <div className="p-4 bg-teal-500/10 border border-teal-500/30 rounded-2xl text-xs text-teal-300 flex items-start justify-between gap-3">
          ...
          <button onClick={() => setAnalysisResult(null)} className="text-zinc-400 hover:text-white">
            ✕
          </button>
        </div>
      )}
